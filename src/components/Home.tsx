...
import { Severity, DeliveryMode, DateSession } from '../types'
...
export default function Home({ onStart }: { onStart: (s: DateSession) => void }) {
  ...
  return (
    <div className="p-4 max-w-md mx-auto">
      ...
      <select value={templateId} onChange={e => onTemplateChange(e.target.value)} className="w-full p-2 my-2 border rounded">
        {templates.map(t => <option key={t.id} value={t.id}>{t.title}</option>)}
      </select>

      <label className="block text-sm font-medium">Sender / Relation</label>
      <select value={relation} onChange={e => { setRelation(e.target.value); setSenderName(e.target.value) }} className="w-full p-2 my-2 border rounded">
      ...
      <input value={senderName} onChange={e => setSenderName(e.target.value)} className="w-full p-2 my-2 border rounded" />
      ...
      <div className="flex gap-2 my-2">
        ...
        <button onClick={() => setSeverity('nuclear')} className={`px-3 py-2 rounded ${severity==='nuclear'?'bg-red-200':'bg-white'}`}>Nuclear</button>
      </div>
      ...
      {deliveryMode === 'scheduled' && (
        <div>
          ...
          <input type="number" value={delay} onChange={e => setDelay(Number(e.target.value))} className="w-full p-2 my-2 border rounded" min={1} />
        </div>
      )}
